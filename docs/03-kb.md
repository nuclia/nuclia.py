...
You can retrieve activity logs for a Knowledge Box, which include different types of events:
...
You can perform more complex queries on logs data using using the query parameter.

### Query Parameters

- `year_month`: The year and month of logs to retrieve (e.g., `2024-02`).
- `show`: Specify fields to display in the output. By default, the following fields are shown: `id`, `date`, `rephrased_question`, `answer`, `resources_count`, `rag_strategies_names`. In any case `id` will be always displayed.
- `filters`: Filter the logs using the following operators:
  - `eq`: equal to (`=`)
  - `gt`: greater than (`>`)
...
- `id`, `user_id`, `user_type`, `client_type`, `total_duration`, `date`, `question`, `rephrased_question`, `answer`, `retrieved_context`, `chat_history`, `filter`, `resources_count`, `learning_id`, `feedback_good`, `feedback_comment`, `model`, `rag_strategies_names`, `rag_strategies`, `status`, `time_to_first_char`, `audit_metadata`.

#### Special Field: `audit_metadata`

The `audit_metadata` field is a customizable dictionary, where you can filter using the `key` operator to target specific keys within the dictionary. For example:
