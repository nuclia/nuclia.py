...
on the other hand if you need an download url that can be shared to download on some other context where you cannot use regular authentication, you can use `temporal_download_url`, and you'll get an url you can use with a simple get during a period of time. When that expires you'll need to request the url again, that will contain a newly generated temporal token. 
