...
LoCoMo Memory Demo
==================
Nuclia SDK example demonstrating NucliaMemory with the LoCoMo
dataset — a benchmark of long-form conversations between two speakers
spanning many sessions, with QA pairs that test memory and recall.

  Paper : https://snap-research.github.io/locomo/
  Data  : https://raw.githubusercontent.com/snap-research/locomo/refs/heads/main/data/locomo10.json

The dataset file (~2.8 MB) is downloaded automatically on first run.
...
Configuration
-------------
  DATASET_PATH  local path for the cached locomo10.json (auto-downloaded)
...
# Local cache path — the file is downloaded here automatically if absent.
DATASET_PATH: Path = Path.home() / ".cache" / "nuclia" / "locomo10.json"
...
# Set to False to run all steps without pausing (e.g. in CI or non-interactive use).
...
def pause(msg: str) -> None:
    """Block until the user presses Enter, unless INTERACTIVE is False."""
    if INTERACTIVE:
...
def show_extracted_facts(
    ...
    if not facts:
        ...
        for fact in reversed(facts):
            ...
            print(f"    • [{ts}] {fact.content.related_entry_ids} {fact.content.text}")
...
def main() -> None:
    ...
    ensure_dataset(DATASET_PATH, DATASET_URL)
